...
- [ ] `G4-TALKBACK-ACTIONS` Execute semantic query, click, type, scroll, drag, focus, key, gesture, wait, assert, snapshot, and replay against live applications. — Exit: the native transport drives each command, observes the resulting app state, rejects stale/disabled targets, and retains deterministic evidence; in-process event emission alone does not satisfy this row
- [ ] `G4-TALKBACK-PIXELS` Provide an explicit native pixel-coordinate fallback. — Exit: a capability-gated platform driver consumes scale-aware coordinates, records ID-derived provenance when present without relabeling the action, executes the fallback, and proves bounds/error behavior; the in-process resolver alone does not satisfy this row
- [ ] `G4-TALKBACK-INSPECT` Ship tree, layout-reason, screenshot, timeline, and capability inspection. — Exit: every inspector is available through the native transport and agrees with retained runtime truth; screenshots and native transport are currently reported unavailable
